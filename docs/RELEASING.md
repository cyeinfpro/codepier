...
.venv/bin/python scripts/build_source_bundle.py --public --output dist/codepier-1.11.0-source.zip
.venv/bin/python scripts/check_release.py --bundle dist/codepier-1.11.0-source.zip
```
...
首次推送后必须核对 GitHub 中真实运行的 CI 结果；本机通过不能冒充远端 CI 通过。随后由维护者确认 `v1.11.0` 标签、发布说明和经过校验的源代码包，再执行公开发布。准备完成与已经发布是两个不同状态。
