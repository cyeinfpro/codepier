'use strict';
function deliveryState(){return S.delivery||(S.delivery={project:'',cursor:'',history:[],next:null});}
const humanBytes=n=>n>=1048576?(n/1048576).toFixed(2)+' MiB':n>=1024?(n/1024).toFixed(1)+' KiB':n+' B';
async function diagnosticsHTML(seq){
  const session=S.session,r=await tool('diagnostics_get',{});
  if(seq!==S.renderSeq||session!==S.session)return '';
  const h=r.hub,buildCard=(title,b,extra='')=>`<article class="panel insight-card"><div class="card-top"><h2>${esc(title)}</h2>${b?.restart_required?'<span class="badge blocked">需重启生效</span>':'<span class="badge completed">运行信息</span>'}</div><dl class="kv"><dt>运行版本</dt><dd>${esc(b?.runtime?.version||'未报告')}</dd><dt>磁盘版本</dt><dd>${esc(b?.disk?.version||'未报告')}</dd><dt>启动时间</dt><dd>${esc(timeText(b?.runtime?.started_at))}</dd><dt>运行源码指纹</dt><dd><code>${esc(b?.runtime?.source_sha256?.slice(0,20)||'未知')}</code></dd><dt>磁盘源码指纹</dt><dd><code>${esc(b?.disk?.source_sha256?.slice(0,20)||'未知')}</code></dd></dl>${extra}</article>`;
  return heading('运行诊断','RUNTIME / OBSERVED STATE','',`<button class="btn ghost" data-action="refresh">${icon('refresh')}刷新</button>`)+
...
async function artifactsHTML(seq){
  const session=S.session,w={...deliveryState()},r=await tool('artifacts_list',{project:w.project,cursor:w.cursor,limit:20});
  if(seq!==S.renderSeq||session!==S.session)return '';
  deliveryState().next=r.next_cursor;
  return heading('产物交付','ARTIFACTS / VERIFIED DELIVERY','',`<button class="btn ghost" data-action="refresh">${icon('refresh')}刷新</button><button class="btn primary" data-insight="register">${icon('plus')}登记产物</button>`)+
    ...
    uiHelp('下载与保留规则','设备在线时可下载。快照保留 7 天，单文件上限 512 MiB，总预算 2 GiB；下载需当前登录或有效授权，不是公开链接。');
}
function bindArtifacts(){const s=$('#artifact-project');if(s)s.onchange=e=>{Object.assign(deliveryState(),{project:e.target.value,cursor:'',history:[]});renderPage(false).catch(err=>toast(err.message,true));};}
function registerArtifact(){
  const projects=S.projects.filter(p=>p.mode==='write');if(!projects.length){toast('请先添加可写项目',true);return;}
  const dialog=modal('登记交付产物',`<form id="artifact-form"><div class="field"><label for="artifact-register-project">项目</label><select name="project" id="artifact-register-project">${projects.map(p=>`<option value="${esc(p.id)}" ${p.id===(deliveryState().project||S.work.project)?'selected':''}>${esc(p.alias)}</option>`).join('')}</select></div><div class="field"><label for="artifact-path">项目内文件路径</label><input id="artifact-path" name="path" required maxlength="1024" placeholder="dist/release.zip"><small>只登记已生成的普通文件；不会自动压缩文件夹或执行构建。</small></div><div class="field"><label for="artifact-name">下载名称（可选）</label><input id="artifact-name" name="name" maxlength="180" placeholder="默认使用原文件名"></div><div class="field"><label for="artifact-source">来源操作编号（可选）</label><input id="artifact-source" name="source_operation_id" maxlength="100"></div><p class="form-note" data-workflow-hint>归档内部是否含凭据需自行检查。文件将复制到独立快照，不会因为原文件后来改变而混用内容。</p></form>`,buttons('artifact-save','生成固定快照'));
  bindWorkflowSubmit(dialog,$('#artifact-form'),$('#artifact-save'),'artifacts_register',()=>Object.fromEntries(new FormData($('#artifact-form'))),async receipt=>{
    const session=S.session;
    const r=await settled(Promise.resolve(receipt));
    if(session!==S.session||!dialog.isConnected)return;
    closeModal(dialog);if(S.page==='artifacts')await renderPage(false);await artifactDetail(r.artifact_id);
  });
}
async function artifactDetail(id){
  const session=S.session,loading=modal('产物详情','<p class="muted">读取文件快照信息…</p>');
  try{
    ...
    modal(a.name,`<dl class="kv"><dt>大小</dt><dd>${humanBytes(a.bytes)}</dd><dt>状态</dt><dd>${a.expired?'已过期':a.device_online?'设备在线':'设备离线，联网后续传'}</dd><dt>SHA-256</dt><dd><code>${esc(a.sha256)}</code></dd><dt>到期时间</dt><dd>${esc(timeText(a.expires))}</dd><dt>产物编号</dt><dd><code>${esc(a.artifact_id)}</code></dd></dl><h3>可靠续传</h3><p class="form-note">命令行下载器会保留 .part 文件、使用 Range 续传并验证完整 SHA-256。令牌必须是对此产物有读取权限的授权；面板自己的产物不会自动交给其他授权。</p><div class="code-box"><pre>${esc(command)}</pre></div><p class="form-note">上面的路径需要替换为实际私有令牌和保存位置。浏览器支持普通下载；是否自动续传取决于浏览器。</p>`,a.expired?'':`<a class="btn primary" href="/api/artifacts/${encodeURIComponent(id)}/download" download>${icon('download')}下载文件</a>`,true);
...
document.addEventListener('click',async e=>{
  const b=e.target.closest('[data-insight]');if(!b||b.disabled||!S.session)return;
  try{
    const w=deliveryState();
    switch(b.dataset.insight){
      case 'trace':await insightTrace(b.dataset.id);break;
      case 'register':registerArtifact();break;
      case 'artifact':await artifactDetail(b.dataset.id);break;
      case 'artifact-next':if(w.next){w.history.push(w.cursor);w.cursor=w.next;await renderPage(false);}break;
