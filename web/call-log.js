...
window.CodePierCallLog = (() => {
  ...
  function rowHTML(r) {
    ...
    return `<details class="call-entry call-${tone}" data-call-id="${esc(r.id)}" ${opened.has(r.id) ? 'open' : ''}><summary aria-label="展开调用 ${esc(r.tool)} ${esc(r.id)}"><span class="call-main"><span class="call-top"><strong class="mono">${esc(r.tool)}</strong><span class="call-kind">${label}</span><span class="call-state">${status(r)}</span></span><span class="call-args">${summary(r)}</span><span class="call-meta"><time>${esc(stamp(r.created))}</time><span>${esc(r.alias || '项目未记录')} · ${esc(r.device_name || '设备未记录')}</span><span>${source(r)}</span><code>#${esc(r.id.slice(0, 12))}</code><span class="call-filter-note" hidden>状态已变化，不再符合当前筛选</span></span></span><span class="call-chevron" aria-hidden="true">⌄</span></summary><div class="call-detail">${saved?.data ? detailHTML(saved.data) : '<p class="muted" role="status">展开后加载参数、输出与执行链路。</p>'}</div></details>`;
  }
