...
async function localSkills(project=S.work.project,workspace_id=workTarget(project).workspace_id||''){
  ...
  async function load(offset=0){
    ...
    try{
      const r=await settled(tool('skills_list',{project,workspace_id,query,source,include_disabled:true,offset,limit:20,...(offset&&catalog?{catalog_sha256:catalog}:{})}));
      ...
      $$('[data-skill-open]',area).forEach(b=>b.onclick=()=>localSkillRead(project,b.dataset.skillOpen,'SKILL.md',0,'',workspace_id).catch(e=>toast(e.message,true)));
...
async function localSkillRead(project,id,resource='SKILL.md',offset=0,sha='',workspace_id=''){
  const session=S.session,loading=modal('读取技能','<p class="muted">读取已选择的本机技能…</p>');
  try{
    const r=await settled(tool('skills_read',{project,workspace_id,skill_id:id,resource_path:resource,explicit:true,offset,max_chars:16000,expected_sha256:sha}));
    ...
    if(r.next_offset!==null)$('#skill-next-page',dialog).onclick=()=>localSkillRead(project,id,resource,r.next_offset,r.sha256,workspace_id).catch(e=>toast(e.message,true));
    $$('[data-skill-resource]',dialog).forEach(b=>b.onclick=()=>localSkillRead(project,id,b.dataset.skillResource,0,'',workspace_id).catch(e=>toast(e.message,true)));
