...
(function initCodePierAppearance(window,document){
  ...
  function valid(value){return VALUES.has(value);}
  function resolve(value){return value==='auto'&&media?.matches?'dark':value==='dark'?'dark':'light';}
  function detail(){return {preference,scheme};}
  ...
  function syncThemeColor(){
    ...
    if(!meta&&document.head){meta=document.createElement('meta');meta.name='theme-color';document.head.append(meta);}
  ...
  function storageChanged(event){
    ...
    else if(event.key===LEGACY_KEY&&!read(KEY))next=valid(event.newValue)?event.newValue:'auto';
    if(next===null)return;
    preference=next;
    apply(true);
  }
