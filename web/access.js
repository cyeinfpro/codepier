...
window.CodePierAccess = (() => {
  const lifetime = CP.createPanel({ owner: () => S.session });
  ...
  function bindProjects(dialog) {
    ...
    if (!all) return;
    const sync = () =>
      $$('[name="project"]', dialog).forEach((field) => {
        field.disabled = all.checked;
      });
  ...
  function selection(dialog = document.querySelector('.modal')) {
    ...
    return {
      all_projects: all,
      projects: all ? [] : $$('[name="project"]:checked', dialog).map((field) => field.value),
    };
  }
  function html() {
  ...
  function bind() {
    const form = $('#access-settings-form'),
      scope = lifetime.mount(form);
    if (!scope) return;
    const login = S.session,
      all = form.elements.all_projects,
      apply = form.elements.apply_to_existing,
      status = $('#access-settings-status', form);
    const sync = () => {
      apply.disabled = !all.checked;
      if (!all.checked) apply.checked = false;
    };
    scope.listen(all, 'change', sync);
    form.onsubmit = (event) => {
      if (!scope.current()) return;
      event.preventDefault();
      busy($('button[type="submit"]', form), async () => {
        const body = {
          all_projects: all.checked,
          developer_scopes: form.elements.developer_scopes.checked,
          apply_to_existing: apply.checked,
        };
        if (
          body.apply_to_existing &&
          !confirm(
            '将当前账号所有有效 OAuth 连接改为可访问全部现有及未来新增项目？\n只扩展项目范围，现有工具权限和凭据保持不变。',
          )
        )
          return;
        const controls = $$('input', form);
        controls.forEach((field) => {
          field.disabled = true;
        });
        status.textContent = '正在保存授权设置…';
        try {
          ...
          for (const name of ['all_projects', 'developer_scopes'])
            form.elements[name].defaultChecked = body[name];
          apply.checked = false;
          apply.defaultChecked = false;
          status.textContent =
            result.note +
            (body.apply_to_existing
              ? ` 已更新 ${result.updated_grants} 个现有 OAuth 连接；无需重新连接。`
              : '');
          toast('授权设置已保存');
        } catch (error) {
          if (scope.current())
            status.textContent =
              error.code === 'NETWORK_UNCERTAIN'
                ? '保存回执暂未收到。请重新读取设置和已有连接核实，不要假定保存失败。'
                : error.message;
        } finally {
          if (scope.current()) {
            controls.forEach((field) => {
  ...
  async function editGrant(id) {
    ...
    const [grant] = await Promise.all([
      api('/api/grants/' + encodeURIComponent(id) + '/projects'),
      loadBasics(),
    ]);
    ...
    const dialog = modal(
      '调整项目范围 · ' + grant.label,
      `<form id="grant-projects-form">${projectFields(S.projects, grant.projects.includes('*'), grant.projects)}<p class="form-note">工具权限保持 ${esc(grant.scopes.join(' / '))}，不会更换凭据或延长有效期。缩小范围后，后续调用和尚未投递的操作会重新核对权限；已开始的任务不会自动停止。</p><p id="grant-projects-status" class="form-note" role="status"></p></form>`,
      `<button class="btn ghost" data-action="close-modal">取消</button><button class="btn primary" type="submit" form="grant-projects-form">保存项目范围</button>`,
    );
    bindProjects(dialog);
    const form = $('#grant-projects-form', dialog),
      status = $('#grant-projects-status', dialog);
    form.onsubmit = (event) => {
      event.preventDefault();
      busy($('button[type="submit"]', dialog), async () => {
        const body = {
          ...selection(dialog),
          expected_projects: grant.projects,
          expected_revision: grant.project_revision,
        };
        const controls = $$('input', form);
        controls.forEach((field) => {
          field.disabled = true;
        ...
        try {
          const response = await api('/api/grants/' + encodeURIComponent(id) + '/projects', {
            method: 'PUT',
            body: JSON.stringify(body),
          });
          ...
          if (S.session === login && dialog.isConnected) status.textContent = error.message;
        } finally {
          if (S.session === login && dialog.isConnected) {
            controls.forEach((field) => {
              field.disabled = false;
            });
            $$('[name="project"]', dialog).forEach((field) => {
              field.disabled = $('[name="all_projects"]', dialog).checked;
            });
          }
        }
      });
  ...
  return { html, bind, detach: lifetime.detach, projectFields, bindProjects, selection, editGrant };
})();
