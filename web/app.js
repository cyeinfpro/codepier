...
function connectHTML(){
  const s=S.settings;
  const grantRow=g=>`<div class="grant-row ${g.revoked?'revoked':''}"><div><h3>${esc(g.label)} <span class="badge ${g.revoked?'neutral':'purple'}">${g.revoked?'已撤销':g.client_id?'OAuth':'PAT'}</span></h3><p>${esc(g.scopes.join(' / '))} · ${esc(g.projects.includes('*')?'全部项目（含未来新增）':g.projects.map(id=>S.projects.find(p=>p.id===id)?.alias||id).join(', '))}<br>到期 ${esc(timeText(g.expires))}</p></div>${!g.revoked?`<div class="actions"><button class="btn small" data-action="edit-grant-projects" data-id="${esc(g.id)}">调整项目范围</button><button class="btn danger small" data-action="revoke-grant" data-id="${esc(g.id)}">撤销</button></div>`:''}</div>`;
...
async function newGrant(){await loadBasics();modal('创建限定范围访问凭据',`<form id="grant-form"><div class="form-row"><div class="field"><label>凭据名称</label><input name="label" required maxlength="80" placeholder="例如 ChatGPT Tunnel"></div><div class="field"><label>有效天数</label><input name="days" type="number" min="1" max="365" value="30" required></div></div>${permissionsHTML(S.projects)}${notice('凭据只显示一次，不会写入浏览器持久存储。适用于 stdio bridge、MCP Inspector 或支持 Bearer 的客户端；ChatGPT 直连使用 OAuth。')}</form>`,buttons('create-grant','创建凭据'));CodePierAccess.bindProjects($('.modal'));$('#create-grant').onclick=()=>busy($('#create-grant'),async()=>{const f=$('#grant-form');if(!f.reportValidity())return;const r=await post('/api/grants',{label:f.elements.label.value,days:Number(f.elements.days.value),scopes:checked('scope'),...CodePierAccess.selection()});modal('保存你的访问凭据',`${notice('复制后存入云端受权限保护的 token 文件。任何持有此凭据的人，都能执行已授权操作。')}<div class="spacer"></div><div class="code-box secret">${esc(r.token)}</div><p class="form-note">到期：${esc(timeText(r.expires))}。需要时可随时撤销。</p>`,`<button class="btn ghost" data-action="close-modal">完成</button><button class="btn" id="copy-token">复制</button><button class="btn primary" id="download-token">下载 token.txt</button>`);$('#copy-token').onclick=()=>copy(r.token);$('#download-token').onclick=()=>download('token.txt',r.token+'\n','text/plain');});}
async function consentModal(id){try{const r=await api('/api/oauth/requests/'+encodeURIComponent(id));modal('确认 MCP 应用授权',`<p class="form-note"><strong>${esc(r.client_name)}</strong> 正在申请访问你的本机项目。</p><dl class="kv"><dt>客户端</dt><dd><code>${esc(r.client_id)}</code></dd><dt>回调地址</dt><dd>${esc(r.redirect_uri)}</dd><dt>资源</dt><dd>${esc(r.resource)}</dd></dl>${permissionsHTML(S.projects,r.scopes,r.access_defaults||{})}${notice('只有你确认的项目范围会被授权；全部项目选项会包含未来新增项目。请核对应用名称、回调地址和工具权限。')}`,`<button class="btn ghost" id="deny-consent">拒绝</button><button class="btn primary" id="allow-consent">允许所选范围</button>`,false,false);CodePierAccess.bindProjects($('.modal'));const decide=allow=>busy(allow?$('#allow-consent'):$('#deny-consent'),async()=>{const out=await post(`/api/oauth/requests/${encodeURIComponent(id)}/decide`,{allow,scopes:checked('scope'),...CodePierAccess.selection()});location.assign(out.redirect);});$('#deny-consent').onclick=()=>decide(false);$('#allow-consent').onclick=()=>decide(true);}catch(e){toast(e.message,true);history.replaceState(null,'',location.pathname+location.hash);}}
