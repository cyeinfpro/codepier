...
function chatStatus(text,error=false){const el=$('#chat-status');if(el){el.textContent=text;el.classList.toggle('is-error',error);}}
...
async function chatSwitch(row,project=row?.project_id||ChatUI.project,options={}){
  const c=ChatUI;chatRememberView();c.dialogCancel?.();chatClosePopover();
  ...
  if(row){chatConnect();const view=chatView();if(view.settingOp&&['sending','queued','claimed'].includes(view.settingOp.state))chatWatchReceipt(row.id,view.settingOp.receipt,c.generation,view,view.settingOp);}if(c.inspector)chatInspector(c.inspector);
...
async function chatPage(){
  ...
  <aside class="chat-sidebar" aria-label="会话历史"><div class="chat-sidebar-head"><span class="chat-wordmark">CodePier<span>码头 · 工作空间</span></span></div><button type="button" id="chat-new" class="chat-new-button" title="新对话" aria-label="新对话">${chatIcon('plus')}<span>新对话</span><kbd data-chat-shortcut="new">⌘ ⇧ O</kbd></button>
  ...
  <footer class="chat-sidebar-footer"><button type="button" id="chat-command-menu" class="chat-sidebar-action">${chatIcon('command')}<span>快捷操作</span><kbd data-chat-shortcut="command">⌘ K</kbd></button><div class="chat-sidebar-preferences"><button type="button" data-nav="overview">${chatIcon('settings')}控制台</button><select id="chat-appearance" aria-label="外观"><option value="auto">跟随系统</option><option value="light">浅色</option><option value="dark">深色</option></select></div></footer></aside>
  <div class="chat-main"><header class="chat-header"><button class="chat-square chat-drawer-toggle" id="chat-history-toggle" aria-label="切换会话历史" title="会话历史 · ⌘ B" aria-expanded="false">${chatIcon('menu')}</button>
  <div class="chat-heading"><h1 tabindex="-1" id="chat-title">新对话</h1><span id="chat-subtitle"></span></div><div class="chat-header-right"><span id="chat-connection" class="chat-connection" title="连接状态"><i></i><span>就绪</span></span>
  ...
  <details class="chat-overflow"><summary aria-label="更多操作">···</summary><div><button data-devtools="overview">当前项目开发工具</button><button data-devtools="validation">测试与验收</button><button data-devtools="handoff">任务衔接</button><button id="chat-rename">重命名</button><button id="chat-resume">恢复会话</button><button id="chat-export">导出完整 Markdown</button><button id="chat-export-json">导出完整 JSON</button><button id="chat-stop">停止会话进程</button><button id="chat-delete" class="danger">删除网页记录</button></div></details></div></header>
  ...
  <div id="chat-scroll" class="chat-scroll" tabindex="0" aria-label="对话消息"><button id="chat-older" class="chat-text-button" hidden>显示更早消息</button><div id="chat-empty" class="chat-empty"><div class="chat-empty-mark">${chatIcon('command')}</div><span class="chat-eyebrow">从一个想法开始</span><h2>今天，<br>做点什么？</h2><p>继续项目，或把新的想法交给 ${c.provider==='codex'?'Codex':'Pi'}。</p><div class="chat-starters"><button data-chat-starter="梳理这个项目的结构，指出值得优先处理的问题。">${chatIcon('folder')}<span><strong>了解项目</strong><small>梳理结构与关键入口</small></span></button><button data-chat-starter="检查当前改动，找出可复现的问题并给出修复建议。">${chatIcon('check')}<span><strong>检查改动</strong><small>找出问题与潜在风险</small></span></button><button data-chat-starter="先阅读项目说明和已有实现，再制定实施计划。">${chatIcon('command')}<span><strong>制定计划</strong><small>把想法拆成具体步骤</small></span></button></div></div><div id="chat-messages" class="chat-messages"></div></div>
  <button id="chat-latest" class="chat-latest" hidden>↓ 回到最新</button>
  <div class="chat-composer-wrap"><div id="chat-new-context" class="chat-new-context"><label>工作项目<select id="chat-project" aria-label="项目">${S.projects.map(p=>`<option value="${esc(p.id)}" ${p.id===c.project?'selected':''}>${esc(p.alias)}</option>`).join('')}</select></label><button type="button" id="chat-change-project" class="chat-text-button" aria-label="搜索并选择工作项目">选择项目</button><span id="chat-target-info"></span></div><div id="chat-activity" class="chat-activity" hidden role="status"><i aria-hidden="true"></i><span id="chat-activity-text"></span><button type="button" id="chat-activity-queue"></button></div><div id="chat-outbox" class="chat-outbox" aria-live="polite"></div><div id="chat-files" class="chat-files" aria-live="polite"></div>
  ...
  <form id="chat-form" class="chat-composer"><textarea id="chat-compose" rows="2" aria-label="消息" aria-describedby="chat-status" placeholder="描述任务，输入 / 查看指令…" maxlength="65536"></textarea>
  <div class="chat-compose-bottom"><div class="chat-compose-controls"><button class="chat-square" type="button" id="chat-attach" aria-label="添加文件或图片" title="上传图片与文件">${chatIcon('plus')}</button><input type="file" id="chat-file-input" multiple hidden><button class="chat-square" type="button" id="chat-commands" aria-label="指令与技能" title="指令与技能">${chatIcon('command')}</button><button class="chat-square" type="button" id="chat-file-library" aria-label="附件库" title="附件库">${chatIcon('file')}</button></div>
  <div class="chat-send-actions"><select id="chat-send-mode" aria-label="发送方式" title="正在执行时如何发送新消息"><option value="followup">排队跟进</option><option value="steer">立即补充</option></select><button type="button" id="chat-interrupt" class="chat-stop-turn" hidden>${chatIcon('stop')}<span>中断当前</span></button><button type="submit" id="chat-send" class="chat-send" aria-label="发送消息">${chatIcon('send')}</button></div></div></form>
  <div class="chat-config-bar"><label class="chat-cli-label"><span>CLI</span><select id="chat-provider" aria-label="原生 CLI"><option value="pi">Pi</option><option value="codex">Codex</option></select></label>
  <button type="button" id="chat-model-picker" class="chat-model-picker" aria-label="选择模型" aria-haspopup="dialog" aria-expanded="false">${chatIcon('model')}<span id="chat-model-name">选择模型</span>${chatIcon('chevron')}</button>
  <label class="chat-effort-label">${chatIcon('brain')}<span>思考</span><select id="chat-effort-select" aria-label="思考强度"><option value="">本机默认</option></select></label>
  <button type="button" id="chat-cwd-button" class="chat-cwd-button" title="工作目录">${chatIcon('folder')}<span>.</span></button><span id="chat-setting-state" class="chat-setting-state"></span><button id="chat-settings-retry" class="chat-text-button" hidden>重试设置</button></div>
  <div class="chat-composer-note"><span id="chat-status" role="status">Enter 发送 · Shift+Enter 换行</span><button id="chat-retry" hidden>重试原请求</button><button id="chat-usage" title="查看用量">上下文</button></div></div>
  </div>
  ...
  <div id="chat-popover" class="chat-popover" role="dialog" aria-label="模型选择" hidden></div><div id="chat-slash" class="chat-slash" role="listbox" aria-label="可用指令" hidden></div></section>`;
...
function chatDrawer(open,switching=false){
  ...
  else {root.classList.toggle('drawer-open',open);$('.chat-main').inert=open;if(open)$('#chat-new').focus();else if(!switching)$('#chat-history-toggle').focus({preventScroll:true});}
...
function chatResizeComposer(){const el=$('#chat-compose');if(el){el.style.height='auto';el.style.height=Math.min(Math.max(72,innerHeight*.22),el.scrollHeight)+'px';chatScheduleLayout();}}
...
function chatRestore(history=true){const c=ChatUI,v=chatView();$('#chat-compose').value=v.draft;$('#chat-provider').value=c.selected?.provider||c.provider;$('#chat-provider').disabled=!!c.selected;$('#chat-title').textContent=c.selected?.title||'新对话';$('#chat-subtitle').textContent=S.projects.find(p=>p.id===c.project)?.alias||'选择项目';$('#chat-empty p').textContent='继续项目，或把新的想法交给 '+(c.provider==='codex'?'Codex':'Pi')+'。';$('#chat-cwd-button span').textContent=c.cwd;$('#chat-cwd-button').title='工作目录：'+(c.selected?.cwd||c.cwd);$('#chat-send').disabled=v.busy;$('#chat-retry').hidden=!v.pending;$('#chat-interrupt').hidden=!v.active;$('#chat-send-mode').value=v.mode;chatResizeComposer();chatFiles();chatOutbox();chatSettings(v.settings);if(history)chatRenderHistory();chatStats();$('#chat-find-input').value=v.search||'';$('#chat-find-count').textContent='';chatTargetSync();chatSyncChrome();}
...
function chatSettings(settings){const v=chatView();if(settings)v.settings=settings;const current=chatModelCurrent(v),model=chatModels(v).find(m=>chatModelKey(m)===current||m.model===current||m.id===current);const name=$('#chat-model-name');if(!name)return;name.textContent=model?.name||model?.displayName||current||(v.catalog?'本机默认':'选择模型');$('#chat-model-picker').title=(model?.provider?model.provider+' / ':'')+(model?.id||current||'发送前可选择模型');
  ...
  const state=$('#chat-setting-state');state.textContent=v.settingOp?(v.settingOp.state==='error'?'设置失败':v.settingOp.state==='uncertain'?'设置待核实':'正在切换…'):v.pendingSettings?'下轮生效':current?(v.configForResume?'恢复时生效':ChatUI.selected?'原生配置':'首条消息生效'):'';$('#chat-settings-retry').hidden=!v.settingOp||!['error','uncertain'].includes(v.settingOp.state);
  ...
  const steer=$('#chat-send-mode option[value="steer"]');if(steer){const caps=v.settings?.capabilities||v.catalog?.capabilities;steer.disabled=caps?.steer===false;}
...
async function chatSend(retry=false){
  ...
  if(v.files.some(f=>!f.ready)){chatStatus('附件仍在上传，或需要重试失败的附件',true);return;}
  ...
  try{
    if(!pending.started){const row=await chatAPI('start',{id:pending.id,cli:pending.cli,mode:'chat',cwd:pending.cwd,...(pending.continue_session?{continue_session:pending.continue_session}:{}),...(pending.model!=null?{model:pending.model}:{}),...(pending.effort!=null?{effort:pending.effort}:{})},project);pending.started=true;pending.row=row;}
    ...
    if(!current)return;if(pending.row){c.selected=pending.row;v.cursor=0;c.cursor=0;v.configForResume=false;v.selectedModel=null;v.selectedEffort=null;chatConnect();chatList();}chatRestore(false);chatStatus(pending.action==='chat_steer'?'补充指令已提交':'已接收，等待本机处理');
  }catch(e){
    if(S.session!==identity)return;
    ...
    if(rejected){v.pending=null;v.outbox.delete(pending.receipt);if(pending.row&&visible()){const key=[...c.views].find(([,view])=>view===v)?.[0];if(key)c.views.delete(key);c.views.set(chatKey(project,pending.id),v);c.selected=pending.row;v.cursor=0;chatConnect();chatRestore(false);}}
...
function chatConnect(){const c=ChatUI,gen=c.generation,id=c.selected?.id,v=chatView();if(!id)return;c.source?.close();c.cursor=v.cursor||0;
  const source=new EventSource('/api/native/sessions/'+encodeURIComponent(id)+'/events?'+new URLSearchParams({project:c.project,cursor:c.cursor}));c.source=source;
  ...
  source.addEventListener('session',e=>{if(!chatCurrent(gen,v)||source!==c.source)return;try{Object.assign(c.selected,JSON.parse(e.data));$('#chat-title').textContent=c.selected.title||'CLI 对话';if(!['running','starting'].includes(c.selected.status)){v.active=null;$('#chat-interrupt').hidden=true;chatConnection('stopped',chatStateName(c.selected.status));chatStatus(c.selected.error||'会话已停止，发送消息可恢复原生历史');}chatSyncChrome();if(c.inspector==='overview')chatInspectorRender();}catch{}});
  ...
  source.onopen=()=>{if(chatCurrent(gen,v)&&source===c.source){chatConnection('online','实时连接');chatStatus('已连接 · Enter 发送');}};
...
function chatEvent(e){
  ...
  if(e.type==='user'){if(!e.parent_receipt&&!e.steering)v.active=e.receipt;v.outbox.delete(e.receipt);chatOutbox();$('#chat-interrupt').hidden=!v.active;}
  ...
  item.data={...item.data,...e};if(e.type==='approval'&&e.resolved&&item.node){item.node.querySelector('.chat-approval-actions')?.remove();chatApproval(item);}
...
function chatRenderHistory(scroll=true){
 ...
 $('#chat-empty').hidden=!!v.order.length;$('#chat-older').hidden=v.order.length<=v.limit;
 if(scroll){if(v.follow!==false)chatBottom();else $('#chat-scroll').scrollTop=v.scrollTop||0;}
 $('#chat-latest').hidden=chatAtBottom();
}
function chatCreateItem(item){if(item.kind==='review'){chatCreateReview(item);return;}const article=document.createElement('article');article.className='chat-message chat-message-'+item.kind;article.dataset.key=item.key;const header=document.createElement('div'),label=document.createElement('span'),actions=document.createElement('div');header.className='chat-message-head';label.className='chat-message-label';label.textContent=({user:item.data.parent_receipt?'你 · 补充指令':'你',assistant:ChatUI.selected?.provider==='codex'?'Codex':'Pi',reasoning:'思考过程',tool:'工具执行',approval:'需要确认',error:'未完成'})[item.kind]||item.kind;actions.className='chat-message-actions';
...
function chatApproval(item){
  ...
  async function answer(value){
    ...
    if(op.busy||op.identity!==S.session||item.data.resolved||item.answered)return;
...
async function chatResume(){
  ...
  v.resumeId=v.resumeId||uid();v.resumeBusy=true;chatSyncChrome();
  try{const next=await chatAPI('start',{id:v.resumeId,cli:row.provider,mode:'chat',cwd:c.cwd,continue_session:row.id,...(v.selectedModel!=null?{model:v.selectedModel}:{}),...(v.selectedEffort!=null?{effort:v.selectedEffort}:{})},project);
    if(S.session!==identity||![...c.views.values()].includes(v))return;
    const current=chatViewVisible(v,identity);v.resumeId=null;v.cursor=0;v.configForResume=false;v.selectedModel=null;v.selectedEffort=null;
...
function chatExport(format){const sid=ChatUI.selected?.id;if(!sid){chatStatus('请先打开一条会话');return;}const a=document.createElement('a');a.href='/api/native/sessions/'+encodeURIComponent(sid)+'/export?format='+format;a.download='conversation.'+format;a.click();chatStatus('正在导出此会话的完整已同步记录');}
function chatFindToggle(open){$('#chat-find-bar').hidden=!open;if(open){$('#chat-find-input').value=chatView().search||'';chatFind();$('#chat-find-input').focus();}else{for(const el of document.querySelectorAll('.chat-message.search-match'))el.classList.remove('search-match');$('#chat-compose').focus();}}
