...
await writeFile('manifest.json',JSON.stringify({sdk:`@modelcontextprotocol/ext-apps@${sdkVersion}`,outputs},null,2));
...
async function collect(directory){
 ...
 for(const entry of entries.sort((a,b)=>a.name.localeCompare(b.name))){
  ...
  for(const name of names){
   ...
   const license=(await readFile(join(path,name),'utf8')).replace(/\r\n?/g,'\n').replace(/[ \t]+$/gm,'').trimEnd();
   text.push('-- '+name+' --\n'+license);
  }
