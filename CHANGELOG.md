...
- 强化无效 URL、重定向原始控制字符、混合私网 DNS 和组播地址检查；按原始读取片段检查传输期限，保留大小、SHA、无覆盖、无自动解压及失败临时文件清理。
...
- 修复原生 CLI 停止期间，单次进程列表查询失败立即将已退出会话标为孤立进程的问题；保留未回收子进程的身份，在原有截止时间内重新确认进程组，只有确认没有存活成员才报告停止成功。
