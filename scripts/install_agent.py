...
def start_service(base, python):
    ...
    if sys.platform == 'darwin':
        ...
        # XML avoids PowerShell interpolation of user-selected filesystem paths.
        import xml.etree.ElementTree as ET
        ns = 'http://schemas.microsoft.com/windows/2004/02/mit/task'
        ET.register_namespace('',ns)
        def child(parent, name, value=None):
            e = ET.SubElement(parent,'{'+ns+'}'+name)
            if value is not None:e.text = value
            return e
        user = subprocess.check_output(['whoami.exe'],text=True).strip()
        task = ET.Element('{'+ns+'}Task',version='1.2')
        child(child(task,'RegistrationInfo'),'URI','\\'+name)
        triggers=child(task,'Triggers'); trigger=child(triggers,'LogonTrigger')
        child(trigger,'Enabled','true'); child(trigger,'UserId',user)
        principal=child(child(task,'Principals'),'Principal'); principal.set('id','Author')
        ...
        child(settings,'MultipleInstancesPolicy','IgnoreNew'); child(settings,'DisallowStartIfOnBatteries','false')
        child(settings,'StopIfGoingOnBatteries','false'); child(settings,'ExecutionTimeLimit','PT0S')
        restart=child(settings,'RestartOnFailure'); child(restart,'Interval','PT1M');child(restart,'Count','3')
        actions=child(task,'Actions'); actions.set('Context','Author'); action=child(actions,'Exec')
        # Keep logs without an intermediary shell, and hide the service console.
        wrapper=runtime/'run-service.py'
        wrapper.write_text('import os,sys,runpy\nfrom pathlib import Path\n'
            'base=Path(__file__).resolve().parent.parent\n'
            'sys.stdout=open(base/"logs"/"stdout.log","a",buffering=1,encoding="utf-8")\n'
            'sys.stderr=open(base/"logs"/"stderr.log","a",buffering=1,encoding="utf-8")\n'
            'os.chdir(base/"runtime")\nsys.argv=["agent","--config",str(base/"config.json"),"run"]\n'
            'runpy.run_module("agent",run_name="__main__")\n',encoding='utf-8')
        windowless=python.with_name('pythonw.exe')
        child(action,'Command',str(windowless if windowless.exists() else python))
        child(action,'Arguments',subprocess.list2cmdline([str(wrapper)]));child(action,'WorkingDirectory',str(runtime))
        target=base/'service.xml';ET.ElementTree(task).write(target,encoding='utf-16',xml_declaration=True)
        run(['schtasks.exe','/Create','/TN',name,'/XML',target,'/F'])
        run(['schtasks.exe','/Run','/TN',name])
        time.sleep(2)
        run(['powershell.exe','-NoProfile','-Command',
             f"if ((Get-ScheduledTask -TaskName '{name}').State -ne 'Running') {{ exit 1 }}"])
    else:
...
def require_idle(base, current):
    """Read-only preflight; no schema migrations, credential output or cancellation."""
    import sqlite3
    ...
    if database.exists():
        try:
            ...
            raise ValueError('Cannot verify Agent task state; stop tasks and inspect the local journal before retrying.') from exc
        if active:
            raise ValueError('Agent has active operations. Finish them first and run this command in the local terminal, not through this Agent.')
    ...
    if native.exists():
        ...
        if sessions:
            raise ValueError('Native terminal sessions are active. Explicitly stop them in the panel before Agent maintenance.')
...
def local_upgrade(base, args, current):
...
def main():
    ...
    try:
        ...
        if config.exists() or runtime.exists():
            ...
            verify_service_ownership(base)
            require_idle(base,current_config)
