...
Run on macOS with: .venv/bin/python scripts/check_native_chat_real.py
...
''')
    ...
    def launch(previous=None):
        ...
        with patch('agent.native_cli.subprocess.Popen', guarded_popen):
            manager.action('start', project_row, {'id': sid, 'cli': cli, 'mode': 'chat',
                           **({'continue_session': previous} if previous else {})})
    ...
    try:
        ...
        setting=uuid.uuid4().hex
        manager.action('chat_settings',project_row,{'id':sid,'receipt':setting,'effort':'low'})
        wait(lambda:completed(sid,setting))
        raw = fixture.png()
        ...
        case['same_native_history'] = (row['native_thread'] == first_native_thread if cli == 'codex'
            else json.loads(row['argv'])[-1] == json.loads(snapshot(manager, sid)[0]['argv'])[-1])
...
def main():
    ...
    evidence = ROOT / 'docs/evidence/chat-ui-20260915' / (
    ...
    result = {'scope': 'actual standalone chat worker and installed native CLI against localhost mock',
              ...
              'command': '.venv/bin/python scripts/check_native_chat_real.py --cli ' + args.cli}
    exit_code = 1
    try:
